-- Migration: add internal_id & public_id to bc_product
-- 1) internal_id: 内部主键 ULID128，对应应用层 Ulid128/BINARY(16)
-- 2) public_id: 对外 ID（PublicId，prefix_ulid，格式：prd_01HN8X5K9G3QRST2VW4XYZ）
...
ALTER TABLE bc_product
    ...
    ADD UNIQUE KEY uk_bc_product_internal_id (internal_id),
    ADD UNIQUE KEY uk_bc_product_tenant_public_id (tenant_id, public_id);
