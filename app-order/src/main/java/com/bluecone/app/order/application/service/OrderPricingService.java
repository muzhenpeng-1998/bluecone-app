...
public class OrderPricingService {
    ...
    public PricingResult priceItems(Long tenantId, Long storeId, List<ConfirmOrderItemDTO> items) {
        ...
        if (skuIds.isEmpty()) {
            throw new BusinessException(CommonErrorCode.BAD_REQUEST, "明细中必须包含 skuId");
        }
        ...
        if (snapshotMap == null || snapshotMap.isEmpty()) {
            throw new BusinessException(CommonErrorCode.BAD_REQUEST, "当前门店无可售商品");
        }
        long totalCents = 0L;
        for (ConfirmOrderItemDTO item : items) {
            if (item == null || item.getSkuId() == null) {
                throw new BusinessException(CommonErrorCode.BAD_REQUEST, "明细中存在无效 SKU");
            }
            StoreSkuSnapshot snapshot = snapshotMap.get(item.getSkuId());
            if (snapshot == null) {
                throw new BusinessException(CommonErrorCode.BAD_REQUEST, "SKU[" + item.getSkuId() + "] 不存在或不可售");
            }
            if (!Boolean.TRUE.equals(snapshot.getAvailable())) {
                throw new BusinessException(CommonErrorCode.BAD_REQUEST, "SKU[" + item.getSkuId() + "] 当前不可售");
            }
            int quantity = item.getQuantity() == null ? 0 : item.getQuantity();
            if (quantity <= 0) {
                throw new BusinessException(CommonErrorCode.BAD_REQUEST, "明细数量必须大于 0");
            }
